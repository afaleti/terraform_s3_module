...
resource "aws_s3_bucket" "generic" {
  depends_on = ["aws_s3_bucket.replica"] # Force replica bucket to be created first if it will be used
  ...
  versioning {
    enabled = "${var.enable_versioning || var.enable_replica}"
  }
  ...
  lifecycle_rule {
    ...
    prefix = ""
    enabled = "${(var.enable_versioning || var.enable_replica) && var.versioning_expire_deletes}"
  ...
  replication_configuration {
    ...
    "rules" {
      ...
      prefix = "${var.replica_prefix}"
      status = "${var.enable_replica ? "Enabled" : "Disabled"}"
    }
  }
